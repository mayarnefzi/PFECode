...
@Component({
  selector: 'app-cellule3-g',
  templateUrl: './cellule3-g.component.html',
  styleUrls: ['./cellule3-g.component.css']
})
export class Cellule3GComponent {
  ...
  constructor(private adminService:AdminService,private route: ActivatedRoute, private siteService: SiteService, private router: Router) {}
  ...
  updateData() {
    this.siteService.getidBycode3G(this.newcellule.codeCellule).subscribe(
      (response: any) => {
        ...
        this.siteService.getcel3GById(this.selectedidCel).subscribe(
          (res: any) => {
            ...
            if (res && res.length > 0) {
              ...
              this.siteService.updatecel3G(this.selectedidCel, updatedCellule).subscribe(
                (updateRes: any) => {
                  Swal.fire({
                    icon: 'success',
                    title: 'Success!',
